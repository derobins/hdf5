...
H5T_copy_reopen(H5T_t *old_dt)
{
    ...
    if (old_dt->sh_loc.type == H5O_SHARE_TYPE_COMMITTED) {
        ...
        if (NULL == (reopened_fo = (H5T_shared_t *)H5FO_opened(H5F_OPEN_OBJECTS(old_dt->sh_loc.file),
            ...
            if (H5FO_insert(H5F_OPEN_OBJECTS(old_dt->sh_loc.file), old_dt->sh_loc.u.loc.oh_addr,
                            new_dt->shared, false) < 0)
                if (H5FO_insert(H5F_OPEN_OBJECTS(old_dt->sh_loc.file), old_dt->sh_loc.u.loc.oh_addr,
                                new_dt->shared, false) < 0)
