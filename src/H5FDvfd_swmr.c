...
H5FD__swmr_reader_open(H5FD_vfd_swmr_t *file)
{
    ...
    file->api_elapsed_ticks = HDcalloc(file->api_elapsed_nbuckets, sizeof(*file->api_elapsed_ticks));
...
swmr_reader_close(H5FD_vfd_swmr_t *file)
{
    ...
    if (file->api_elapsed_ticks != NULL) {
        ...
        for (i = 0; i < file->api_elapsed_nbuckets; i++) {
            hlog_fast(swmr_stats, "%s: %" PRIu32 " ticks elapsed in API %" PRIu64 " times", __func__, i,
                      file->api_elapsed_ticks[i]);
        }
        HDfree(file->api_elapsed_ticks);
    }
                   ...
                   size_t size, void *const buf /*out*/)
{
    ...
    hlog_fast(swmr_read,
              "%s: enter type %d addr %" PRIuHADDR " size %zu "
              "file %s",
              __func__, type, addr, size, (entry == NULL) ? "lower" : "shadow");
    ...
    if (!file->pb_configured) {
        hlog_fast(swmr_read_exception, "%s: skipping checksum, page buffer not configured", __func__);
    }
    else if (entry->length != init_size) {
        hlog_fast(swmr_read_exception, "%s: skipping checksum, buffer size != entry size", __func__);
    }
        ...
        hlog_fast(swmr_read_err, "addr %" PRIuHADDR " page %" PRIuHADDR " len %zu type %d ...", addr,
                  addr / fs_page_size, init_size, type);
        hlog_fast(swmr_read_err,
                  "... index[%" PRId64 "] lower pgno %" PRIu64 " shadow pgno %" PRIu64 " len %" PRIu32
                  " sum %" PRIx32,
                  (int64_t)(entry - index), entry->hdf5_page_offset, entry->md_file_page_offset,
                  entry->length, entry->chksum);
        ...
        hlog_fast(swmr_read_err, "... header tick last read %" PRIu64 " latest %" PRIu64,
                  file->md_header.tick_num, tmp_header.tick_num);
