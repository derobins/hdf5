...
hlog_init(void)
{
    ...
    while ((item = strsep(&settings, " ,")) != NULL) {
        ...
        if (strcmp(val, "on") == 0 || strcmp(val, "yes") == 0)
            ...
            warnx("%s: bad HLOG value \"%s\" in item \"%s\"", __func__, val, item);
            continue;
        }

        if (hlog_set_state(key, state, true) == -1) {
            warn("%s: could not set state for HLOG item \"%s\"", __func__, item);
        }
    }
...
{
    struct hlog_outlet *ls;
    if ((ls = hlog_outlet_lookup(ls_arg->ls_name)) == NULL || ls->ls_rendezvous) {
        TAILQ_INSERT_TAIL(&hlog_outlets, ls_arg, ls_next);
        if (ls == NULL)
            return;
        warnx("%s: rendezvous with log-outlet '%s'", __func__, ls->ls_name);
