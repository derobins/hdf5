...
#ifndef _printflike
#define _printflike(_fmt, _args) __attribute__((__format__(__printf__, _fmt, _args)))
#endif

enum hlog_outlet_state { HLOG_OUTLET_S_PASS = 0, HLOG_OUTLET_S_OFF = 1, HLOG_OUTLET_S_ON = 2 };
...
#define HLOG_CONSTRUCTOR(__sym)                                                                              \
    ...
    void hlog_constructor_##__sym(void) { hlog_outlet_register(&__sym); }                                    \
    void hlog_undefined_##__sym(void) _constructor

#define HLOG_OUTLET_FOREACH(__le, __le0) for ((__le) = (__le0); (__le) != NULL; (__le) = (__le)->ls_parent)
...
#define HLOG_OUTLET_DEFN(__sym, __name, __parent, __state)                                                   \
    struct hlog_outlet __sym = {.ls_name = __name, .ls_parent = (__parent), .ls_state = (__state)};          \
    HLOG_CONSTRUCTOR(__sym)

#define HLOG_OUTLET_MEDIUM_DEFN(__name, __parent, __state)                                                   \
    HLOG_OUTLET_DEFN(HLOG_PREFIX(__name), #__name, &HLOG_PREFIX(__parent), __state)

#define HLOG_OUTLET_SHORT_DEFN(__name, __parent) HLOG_OUTLET_MEDIUM_DEFN(__name, __parent, HLOG_OUTLET_S_PASS)

#define HLOG_OUTLET_TOP_DEFN(__name) HLOG_OUTLET_DEFN(HLOG_PREFIX(__name), #__name, NULL, HLOG_OUTLET_S_PASS)

HLOG_OUTLET_DECL(all);

#define hlog(_name, _fmt, ...) hlog_impl(&HLOG_PREFIX(_name), _fmt, __VA_ARGS__)
...
void hlog_always(struct hlog_outlet *, const char *, ...) _printflike(2, 3);

void hlog_impl(struct hlog_outlet *, const char *, ...) _printflike(2, 3);
