...
static herr_t
defer_free(H5F_shared_t *shared, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size)
{
    lower_defree_t *df;

    if ((df = HDmalloc(sizeof(*df))) == NULL)
        return FAIL;
    ...
    hlog_fast(h5mf_defer, "%s.%d: deferred free at %" PRIuHADDR ", %" PRIuHSIZE " bytes until tick %" PRIu64,
              __FILE__, __LINE__, addr, size, df->free_after_tick);
...
H5MF_process_deferred_frees(H5F_t *f, const uint64_t tick_num)
{
    ...
    while ((df = SIMPLEQ_FIRST(&defrees)) != NULL) {
        if (tick_num <= df->free_after_tick)
            break;
        hlog_fast(h5mf_defer, "%s.%d: processing free at %" PRIuHADDR ", %" PRIuHSIZE " bytes", __FILE__,
                  __LINE__, df->addr, df->size);
    ...
    if (err != SUCCEED) {
        hlog_fast(h5mf_defer, "%s.%d: error: dropped entries on the floor", __FILE__, __LINE__);
    }
...
H5MF_alloc(H5F_t *f, H5FD_mem_t alloc_type, hsize_t size)
{
    ...
    hlog_fast(h5mf_alloc, "%s: enter %p type %u size %" PRIuHSIZE " tick %" PRIu64, __func__,
              (void *)f->shared, (unsigned)alloc_type, size,
    ...
    hlog_fast(h5mf_alloc, "%s: leave %p type %u addr %" PRIuHADDR " size %" PRIuHSIZE, __func__,
              (void *)f->shared, (unsigned)alloc_type, ret_value, size);
...
H5MF_xfree(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size)
{
    ...
    hlog_fast(h5mf_free, "%s: Entering - alloc_type %u addr %" PRIuHADDR " size %" PRIuHSIZE " tick %" PRIu64,
...
H5MF__xfree_inner_impl(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size)
{
    ...
    hlog_fast(h5mf_free, "%s: enter %p type %u addr %" PRIuHADDR " size %" PRIuHSIZE, __func__,
              (void *)f->shared, (unsigned)alloc_type, addr, size);
    ...
    if (!f->shared->fs_man[fs_type]) {
        ...
        hlog_fast(h5mf_free, "%s: fs_addr %" PRIuHADDR, __func__, f->shared->fs_addr[fs_type]);
        ...
        if (!H5F_addr_defined(f->shared->fs_addr[fs_type])) {
            ...
            if ((status = H5MF_try_shrink(f, alloc_type, addr, size)) < 0)
                ...
                hlog_fast(h5mf_free, "%s: dropping addr %" PRIuHADDR " size %" PRIuHSIZE " on the floor!",
                          __func__, addr, size);
                HGOTO_DONE(SUCCEED)
            }
        }
...
H5MF_try_extend(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size, hsize_t extra_requested)
{
    ...
    if (allow_extend) {
        ...
        hlog_fast(h5mf_extend, "%s: extended %s", __func__, htri_to_string(ret_value));
        ...
        if (ret_value == FALSE && (f->shared->fs_strategy == H5F_FSPACE_STRATEGY_FSM_AGGR ||
            ...
            hlog_fast(h5mf_extend, "%s: H5MF__aggr_try_extend %s", __func__, htri_to_string(ret_value));
        }
        ...
        if (ret_value == FALSE &&
            ...
            if (f->shared->fs_man[fs_type]) {
                ...
                hlog_fast(h5mf_extend, "%s: Try to H5FS_sect_try_extend %s", __func__,
            ...
            if (ret_value == FALSE && H5F_PAGED_AGGR(f) && map_type != H5FD_MEM_DRAW) {
                ...
                hlog_fast(h5mf_extend, "%s: Try to extend into the page end threshold %s", __func__,
                          htri_to_string(ret_value));
            }
        }
    }
...
H5MF_try_shrink(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size)
{
    ...
    hlog_fast(h5mf_shrink, "%s: Entering - alloc_type %u addr %" PRIuHADDR " size %" PRIuHSIZE, __func__,
    ...
    hlog_fast(h5mf_shrink, "%s: Leaving, ret_value %d", __func__, ret_value);
...
H5MF_close(H5F_t *f)
{
    ...
    hlog_fast(h5mf, "%s: total deferred frees %" PRIu64, __func__, H5MF_total_deferred_frees(f->shared));
