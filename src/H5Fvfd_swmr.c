...
HLOG_OUTLET_MEDIUM_DEFN(noisy_shadow_defrees, shadow_defrees, HLOG_OUTLET_S_OFF);
...
H5F_vfd_swmr_init(H5F_t *f, hbool_t file_create)
{
    ...
    if (H5F_SHARED_INTENT(shared) & H5F_ACC_RDWR) {
        ...
        hlog_fast(tick, "%s first tick %" PRIu64, __func__, shared->tick_num);
...
H5F_update_vfd_swmr_metadata_file(H5F_t *f, uint32_t num_entries, H5FD_vfd_swmr_idx_entry_t *index)
{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        hlog_fast(noisy_shadow_defrees, "shadow index %" PRIu32 " page offset %" PRIu64 " -> %" PRIuHADDR, i,
                  index[i].md_file_page_offset * shared->fs_page_size, md_addr);
   ...
   "writing index[%d] fo/mdfo/l/chksum/fc/lc = %lld/%lld/%ld/%lx/%lx/%lx\n",
    ...
    TAILQ_FOREACH_REVERSE_SAFE(shadow_defree, &shared->shadow_defrees, shadow_defree_queue, link, prev)
    {
        ...
        hlog_fast(noisy_shadow_defrees, "released %" PRIu32 " bytes at %" PRIu64, shadow_defree->length,
                  shadow_defree->offset);
    ...
    for (i = j = ndeleted = 0; i < nentries; i++) {
        ...
        if (ie->clean) {
            hlog_fast(shadow_index_reclaim,
                      "Visiting clean shadow index slot %" PRIu32 " lower page %" PRIu64
                      " last flush %" PRIu64 " ticks ago",
        ...
        if (ie->clean && ie->tick_of_last_flush + max_lag < tick_num) {
            ...
            hlog_fast(shadow_index_reclaim, "Reclaiming shadow index slot %" PRIu32 " lower page %" PRIu64, i,
                      ie->hdf5_page_offset);
...
update_eot:
    ...
    hlog_fast(eot, "%s leave tick %" PRIu64 " idx len %" PRIu32, __func__, shared->tick_num,
              shared->mdf_idx_entries_used);
...
H5F_vfd_swmr_reader_end_of_tick(H5F_t *f, bool entering_api)
{
    ...
    hlog_fast(eot, "%s enter file %p index len %" PRIu32 " used %" PRIu32, __func__, (void *)file,
    ...
    hlog_fast(tick, "%s last tick %" PRIu64 " new tick %" PRIu64, __func__, shared->tick_num, tmp_tick_num);
    ...
    if (tmp_tick_num != shared->tick_num) {
            ...
            if (oent->hdf5_page_offset == nent->hdf5_page_offset) {
                ...
                if (oent->md_file_page_offset != nent->md_file_page_offset) {
                    ...
                    hlog_fast(shadow_index_update,
                              "shadow page for slot %" PRIu32 " lower page %" PRIu64 " moved, %" PRIu64
                              " -> %" PRIu64,
                              i, oent->hdf5_page_offset, oent->md_file_page_offset,
                 ...
                 */
                hlog_fast(shadow_index_update,
                          "writer removed shadow index slot %" PRIu32 " for page %" PRIu64, i,
                ...
                hlog_fast(shadow_index_update, "writer added shadow index slot %" PRIu32 " for page %" PRIu64,
                          j, nent->hdf5_page_offset);
        ...
        for (; j < new_mdf_idx_entries_used; j++) {
            ...
            hlog_fast(shadow_index_update, "writer added shadow index slot %" PRIu32 " for page %" PRIu64, j,
                      nent->hdf5_page_offset);
            change[nchanges].pgno   = nent->hdf5_page_offset;
            change[nchanges].length = nent->length;
            nchanges++;
        ...
        for (; i < old_mdf_idx_entries_used; i++) {
            ...
            hlog_fast(shadow_index_update, "writer removed shadow index slot %" PRIu32 " for page %" PRIu64,
                      i, oent->hdf5_page_offset);
        ...
        for (i = 0; i < nchanges; i++) {
            hlog_fast(mdc_invalidation,
                      "invalidating MDC entries at page %" PRIu64 " length %" PRIu32 " tick %" PRIu64,
    ...
    hlog_fast(eot,
              "%s exit tick %" PRIu64 " len %" PRIu32 " -> %" PRIu32 " used %" PRIu32 " -> %" PRIu32
              " added %" PRIu32 " removed %" PRIu32 " moved %" PRIu32 " %s",
              __func__, shared->tick_num, shared->old_mdf_idx_len, shared->mdf_idx_len,
              shared->old_mdf_idx_entries_used, shared->mdf_idx_entries_used, entries_added, entries_removed,
              entries_moved, (ret_value == SUCCEED) ? "success" : "failure");
...
insert_eot_entry(eot_queue_entry_t *entry_ptr)
{
    ...
    hlog_fast(eotq,
              "%s: entry %p after %p file %p "
              "tick %" PRIu64 " ending %jd.%09ld",
              __func__, (void *)entry_ptr, (void *)prec_ptr, (void *)entry_ptr->vfd_swmr_file,
              entry_ptr->tick_num, (intmax_t)entry_ptr->end_of_tick.tv_sec, entry_ptr->end_of_tick.tv_nsec);
...
H5F_vfd_swmr_update_entry_eot(eot_queue_entry_t *entry)
{
    ...
    hlog_fast(eotq,
              "%s: updating entry %p file %p "
              "tick %" PRIu64 " ending %jd.%09ld",
              __func__, (void *)entry, (void *)entry->vfd_swmr_file, entry->tick_num,
              (intmax_t)entry->end_of_tick.tv_sec, entry->end_of_tick.tv_nsec);

    HDassert(entry->vfd_swmr_writer == shared->vfd_swmr_writer);
    entry->tick_num    = shared->tick_num;
    entry->end_of_tick = shared->end_of_tick;

    hlog_fast(eotq, "%s: ... to tick %" PRIu64 " ending %jd.%09ld", __func__, entry->tick_num,
              (intmax_t)entry->end_of_tick.tv_sec, entry->end_of_tick.tv_nsec);
...
H5F_vfd_swmr_remove_entry_eot(H5F_t *f)
{
    ...
    if (curr != NULL) {
        hlog_fast(eotq,
                  "%s: entry %p file %p "
                  "tick %" PRIu64 " ending %jd.%09ld",
                  __func__, (void *)curr, (void *)curr->vfd_swmr_file, curr->tick_num,
                  (intmax_t)curr->end_of_tick.tv_sec, curr->end_of_tick.tv_nsec);
...
H5F__vfd_swmr_update_end_of_tick_and_tick_num(H5F_shared_t *shared, hbool_t incr_tick_num)
{
    ...
    if (incr_tick_num) {
        ...
        hlog_fast(tick, "%s tick %" PRIu64 " -> %" PRIu64, __func__, shared->tick_num - 1, shared->tick_num);
...
vfd_swmr_enlarge_shadow_index(H5F_t *f)
{
     ...
     */
    HDmemcpy(new_mdf_idx, old_mdf_idx, sizeof(new_mdf_idx[0]) * old_mdf_idx_len);
...
H5F_vfd_swmr_process_eot_queue(bool entering_api)
{
    ...
    do {
        ...
        if (HDclock_gettime(CLOCK_MONOTONIC, &now) < 0) {
            HGOTO_ERROR(H5E_FUNC, H5E_CANTGET, FAIL, "can't get time via clock_gettime");
        }
