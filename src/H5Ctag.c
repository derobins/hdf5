...
{
    H5C_tag_info_t *tag_info = NULL; /* tag to be re-tagged */
    ...
    if (tag_info) {
        /* Remove the old tag from the hash table */
        HASH_DELETE(hh, cache->tag_list, tag_info);

        /* Change to new tag */
        tag_info->tag = dest_tag;

        /* Re-insert tag info into the hash table */
        HASH_ADD(hh, cache->tag_list, tag, sizeof(haddr_t), tag_info);
    }
