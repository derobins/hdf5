...
H5MV_alloc(H5F_t *f, hsize_t size)
{
    ...
    hlog_fast(h5mv, "%s: leave addr %" PRIuHADDR " size %" PRIuHSIZE, __func__, ret_value, size);
...
H5MV_free(H5F_t *f, haddr_t addr, hsize_t size)
{
    ...
    hlog_fast(h5mv, "%s: enter addr %" PRIuHADDR " size %" PRIuHSIZE, __func__, addr, size);
    ...
    if (shared->fs_man_md == NULL) {
        ...
        hlog_fast(h5mv, "%s: trying to avoid starting up free space manager", __func__);
        ...
        if (shared->fs_state_md == H5F_FS_STATE_DELETING) {
            hlog_fast(h5mv, "%s: dropping addr %" PRIuHADDR " size %" PRIuHSIZE " on the floor!", __func__,
                      addr, size);
            HGOTO_DONE(SUCCEED)
        }
    ...
    hlog_fast(h5mv, "%s: before H5FS_sect_add, addr %" PRIuHADDR " size %" PRIuHSIZE, __func__, addr, size);
