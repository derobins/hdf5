    ...
    if (pb_ptr) {
        ...
        for (i = 0; i < H5PB__HASH_TABLE_LEN; i++) {
            ...
            while (entry_ptr) {
                ...
                flush_ptr = entry_ptr;
                entry_ptr = entry_ptr->ht_next;
                hlog_fast(pbflush, "%s: visiting %zu-byte page %" PRIu64, __func__, flush_ptr->size,
                          flush_ptr->page);
                ...
                if (flush_ptr->is_dirty) {
                    ...
                    if (flush_ptr->delay_write_until != 0) {
                        hlog_fast(pbflush,
                                  "%s: delaying %zu-byte page %" PRIu64 " until %" PRIu64 " (now %" PRIu64
                                  ")",
                                  __func__, flush_ptr->size, flush_ptr->page, flush_ptr->delay_write_until,
                                  shared->tick_num);
                        continue;
                    }
...
H5PB_log_access_by_size_counts(const H5PB_t *pb)
{
    ...
    hlog_fast(pb_access_sizes, "page buffer %p metadata accesses by size:", (const void *)pb);

    for (lo = 0, hi = pb->page_size, i = 0; i < nslots - 1; i++, lo = hi + 1, hi *= 2) {
        hlog_fast(pb_access_sizes, "%16" PRIu64 " accesses %8zu - %8zu bytes long", pb->access_size_count[i],
                  lo, hi);
    }

    hlog_fast(pb_access_sizes, "%16" PRIu64 " accesses %8zu -  greater bytes long", pb->access_size_count[i],
              lo);
}
...
H5PB_read(H5F_shared_t *shared, H5FD_mem_t type, haddr_t addr, size_t size, void *buf /*out*/)
{
    ...
    hlog_fast(pbrd, "%s %p type %d %" PRIuHADDR " size %zu", __func__, (void *)shared, type, addr, size);
                            ...
                            uint32_t *idx_ent_not_in_tl_ptr, uint32_t *idx_ent_not_in_tl_flushed_ptr)
{
    ...
    for (i = 0; i < shared->mdf_idx_entries_used; i++) {
        ...
        if (entry == NULL || !entry->is_dirty) {
            hlog_fast(shadow_index_reclaim, "Marking shadow index slot %" PRIu32 " clean at tick %" PRIu64, i,
                      tick_num);
...
H5PB_write(H5F_shared_t *shared, H5FD_mem_t type, haddr_t addr, size_t size, const void *buf)
{
    ...
    hlog_fast(pbwr, "%s %p type %d addr %" PRIuHADDR " size %zu", __func__, (void *)shared, type, addr, size);
 ...
 */
static herr_t
H5PB__evict_entry(H5F_shared_t *shared, H5PB_entry_t *entry_ptr, bool force, bool only_mark)
{
...
H5PB__flush_entry(H5F_shared_t *shared, H5PB_t *pb_ptr, H5PB_entry_t *const entry_ptr)
{
    ...
    hlog_fast(pbflush_entry, "%s: flushing %zu-byte page %" PRIu64 " @ %" PRIuHADDR, __func__,
              entry_ptr->size, entry_ptr->page, entry_ptr->addr);
    ...
    if (size > pb_ptr->page_size) {
        ...
        if (entry_ptr != NULL && entry_ptr->size < size) {
            ...
            hlog_fast(lengthen_pbentry,
                      ...
                      "last page %" PRIu64 "\n",
                      page, entry_ptr->size, size, last_page);
