...
static herr_t H5FD_family_delete(const char *filename, hid_t fapl_id);
		 ...
		 haddr_t maxaddr)
{
    ...
    if(H5P_FILE_ACCESS_DEFAULT == fapl_id) {
        ...
        if(NULL == (plist = (H5P_genplist_t *)H5I_object(fapl_id)))
            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "not a file access property list")
        if(NULL == (fa = (const H5FD_family_fapl_t *)H5P_peek_driver_info(plist)))
            HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, NULL, "bad family VFD driver info")
    ...
    if(!HDstrcmp(memb_name, temp))
        HGOTO_ERROR(H5E_FILE, H5E_FILEEXISTS, NULL, "provided file name cannot generate unique sub-files")
 ...
 */
static herr_t
H5FD_family_delete(const char *filename, hid_t H5_ATTR_UNUSED fapl_id)
{
    H5P_genplist_t              *plist;
    const H5FD_family_fapl_t    *fa;
    ...
    if(NULL == (member_name = (char *)H5MM_malloc(H5FD_FAM_MEMB_NAME_BUF_SIZE)))
        HGOTO_ERROR(H5E_IO, H5E_CANTALLOC, FAIL, "unable to allocate member name")
    if(NULL == (temp = (char *)H5MM_malloc(H5FD_FAM_MEMB_NAME_BUF_SIZE)))
        HGOTO_ERROR(H5E_IO, H5E_CANTALLOC, FAIL, "unable to allocate temporary member name")
    ...
    if(!HDstrcmp(member_name, temp))
        HGOTO_ERROR(H5E_IO, H5E_CANTDELETEFILE, FAIL, "provided file name cannot generate unique sub-files")
    ...
    while(1) {
        ...
        if(FAIL == delete_error) {
            if(0 == current_member)
                HGOTO_ERROR(H5E_IO, H5E_CANTDELETEFILE, FAIL, "unable to delete member file")
            else
                H5E_clear_stack(NULL);
            break;
        }
